/**
 * POST: add PEARLS to challenge prize pool (anyone can contribute). Body: { initData, amount }
 */
...
export async function POST(
  ...
  if (!Number.isFinite(amt) || amt < LEAGUE_CHALLENGE_MIN_CONTRIBUTION) {
    return NextResponse.json({ error: `Minimum contribution is ${LEAGUE_CHALLENGE_MIN_CONTRIBUTION.toLocaleString()} PEARLS` }, { status: 400 });
  }
  ...
  const challenge = await prisma.leagueChallenge.findUnique({ where: { id: challengeId } });
  ...
  await prisma.leagueChallengeContribution.create({
